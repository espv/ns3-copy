...
NetDeviceContainer SetupWirelessNetwork (NodeContainer& n) 
{
    ...
    WifiMacHelper wifiMac; //WifiMacHelper wifiMac;
...
main (int argc, char *argv[])
{
    ...
    CommandLine cmd;
    cmd.AddValue ("placement_policy", "Sender node number", placementPolicy);
    cmd.AddValue ("NumberOfCepEvents", "the number_of_eventss to be generated by each datasource", numberOfCepEvents);
  ...
  //
    ...
    Ipv4Address sinkAddress = iface.GetAddress (0);
    ...
    for(uint32_t i = 0; i < numNodes; i++)
    {
        ...
        else if ((i == (numNodes-1)) || (i == (numNodes-2)))
        {
            ...
            dcepApps.Get(i)->SetAttribute("event_code", UintegerValue (eventCode++));
            dcepApps.Get(i)->SetAttribute("number_of_eventss", UintegerValue (numberOfCepEvents));
        }
    }
    
      dcepApps.Start (Seconds (1.0));
      dcepApps.Stop (Seconds (2000.0));

      Simulator::Stop(Seconds(3335.0));
      Simulator::Run ();
      Simulator::Destroy ();
