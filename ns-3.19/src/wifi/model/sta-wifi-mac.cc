...
TypeId StaWifiMac::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::StaWifiMac").SetParent<RegularWifiMac>().AddConstructor<
					StaWifiMac>().AddAttribute("ProbeRequestTimeout",
					...
					MakeTraceSourceAccessor(&StaWifiMac::m_assocLogger)).AddTraceSource(
...
void StaWifiMac::SetMaxMissedBeacons(uint32_t missed) {
	NS_LOG_FUNCTION (this << missed);
	m_maxMissedBeacons = missed;
}

void StaWifiMac::SetProbeRequestTimeout(Time timeout) {
	NS_LOG_FUNCTION (this << timeout);
	m_probeRequestTimeout = timeout;
}

void StaWifiMac::SetAssocRequestTimeout(Time timeout) {
	NS_LOG_FUNCTION (this << timeout);
	m_assocRequestTimeout = timeout;
}

void StaWifiMac::StartActiveAssociation(void) {
	NS_LOG_FUNCTION (this);
...
void StaWifiMac::SendAssociationRequest(void) {
...
void StaWifiMac::ProbeRequestTimeout(void) {
	NS_LOG_FUNCTION (this);
	SetState(WAIT_PROBE_RESP);
	SendProbeRequest();
}
...
bool StaWifiMac::IsAssociated(void) const {
...
void StaWifiMac::Enqueue(Ptr<const Packet> packet, Mac48Address to) {
...
void StaWifiMac::Receive(Ptr<Packet> packet, const WifiMacHeader *hdr) {
...
HtCapabilities StaWifiMac::GetHtCapabilities(void) const {
