	...
	if (m_currentLocation->lc) {
		...
		if (!m_currentLocation->program->hasDequeue
			...
			if (curLc->serviceQueues) {
				std::vector<
					std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *>::iterator it =
			...
			if (curLc->emptyQueues != NULL)
				...
				if (!m_currentLocation->program->hasInternalLoop) {
					...
					if (curLc->serviceQueues) {
						...
						std::vector<
							std::queue<
							std::pair<Ptr<SEM>, ProgramLocation> > *>::iterator it =
							curLc->serviceQueuesServed.begin();
						for (;
								it != curLc->serviceQueuesServed.end()
								&& (*it)->empty(); it++)
...
bool Thread::HandleQueueEvent(ExecutionEvent* e) {
	...
	if (qe->enqueue) {
		...
		if (qe->serviceQueue) {
			...
			qe->servQueue->push(
					std::pair<Ptr<SEM>, ProgramLocation>(semToEnqueue,
						m_programStack.top()));
		...
		if (qe->serviceQueue) {
			...
			std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *queueToServe =
				(qe->servQueue == NULL) ?
			...
			ProgramLocation newPl = queueToServe->front().second;
...
//	std::cout << "START PROCESSING " << this->m_pid << " " << Simulator::Now().GetNanoSeconds() << std::endl;
}
...
//	std::cout << "STOP PROCESSING " << this->m_pid << " " << Simulator::Now().GetNanoSeconds() << std::endl;
