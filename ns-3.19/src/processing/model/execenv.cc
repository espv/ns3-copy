	...
	if (!tokens[1].compare("FIFO")) {
		...
		if (!tokens[3].compare("services")) {
			...
			this->serviceQueues[tokens[0]] = new std::queue<
					std::pair<Ptr<SEM>, ProgramLocation> >();
...
bool ExecEnv::queuesIn(std::string first, std::string last, LoopCondition *lc) {
	if(lc->stateQueues) {
		...
		std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *firstQueue =
				serviceQueues[first];
		std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *lastQueue =
				serviceQueues[last];

		std::vector<std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *>::iterator qIt =
				std::find(serviceQueueOrder.begin(), serviceQueueOrder.end(),
						firstQueue);
		std::vector<std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *>::iterator qItLast =
				std::find(serviceQueueOrder.begin(), serviceQueueOrder.end(),
						lastQueue);
...
//				!tokens[1].compare("ENQUEUEN")) {
			...
			if (!tokens[4].compare("0")) {
				if (currentlyHandled->lc == NULL) {
					...
					if (currentlyHandled->lc->serviceQueues) {
						...
						std::map<
								std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *,
	...
	if (!tokens[1].compare("CALL") || !tokens[1].compare("LOOP")) {
		...
		e->sem = !tokens[2].compare("0") ? NULL : m_serviceMap[tokens[2]];
		execEvent = e;
