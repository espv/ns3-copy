...
class Condition : public ExecutionEvent {
 public:
  ...
  Callback<uint32_t, std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *, std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *> getServiceConditionQueues;
...
class LoopCondition : public Condition {
 public:
  ...
  std::vector<std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *> serviceQueuesServed;
...
public:
	...
	bool local;
	Ptr<Queue> queue;
	std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *servQueue;
	std::string queueName;
