...
//    	std::cout << pi->remaining[NANOSECONDS].amount << " = (" << pi->remaining[CYCLES].amount << "/" << m_freq << ") * 1000" << std::endl;
    ...
    // std::cout << "Consumed " << NanoSeconds(pi->remaining[NANOSECONDS].amount) << " (" << pi->remaining[CYCLES].amount << " cycles) on " << this->m_name << std::endl;

	// Finally, schedule the completion function to run
	// after the computed duration.
	pi->processingCompleted = Simulator::Schedule(
			NanoSeconds(pi->remaining[NANOSECONDS].amount),
...
bool CPU::Interrupt(InterruptRequest ir) {
	...
	if (!interruptsEnabled)
		...
		ee->serviceQueues[this->hirqQueue]->push(
				std::pair<Ptr<SEM>, ProgramLocation>(ir.service,
						newProgramLocation));

        NS_ASSERT_MSG(ee->serviceQueues[this->hirqQueue],
                "Unable to find interrupt handler named " << this->hirqHandler);

		ProgramLocation irqProgramLocation;
