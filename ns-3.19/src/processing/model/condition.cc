...
uint32_t ConditionFunctions::ServiceQueueCondition(std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *first, std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *last)
{
	if(first == last)
		return first->empty();
	else {
		std::vector<std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *>  *serviceQueueOrder = &node->GetObject<ExecEnv> ()->serviceQueueOrder;
		std::vector<std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *> ::iterator firstFound =
				 std::find(serviceQueueOrder->begin(), serviceQueueOrder->end(), first);
		std::vector<std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *>::iterator lastFound =
				 std::find(serviceQueueOrder->begin(), serviceQueueOrder->end(), last);

		std::vector<std::queue<std::pair<Ptr<SEM>, ProgramLocation> > *>::iterator it = serviceQueueOrder->begin();
