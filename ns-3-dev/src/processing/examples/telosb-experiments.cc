...
Ptr<ProtocolStack> ps = CreateObject<ProtocolStack>();
...
#define CC2420_THROUGHPUT_EXPERIMENT 0
    ...
    /*netDevice1->node_id = 1;
    netDevice2->node_id = 2;
    netDevice3->node_id = 3;*/
  ...
  Ptr<ExecEnvHelper> eeh = CreateObjectWithAttributes<ExecEnvHelper>(
    ...
    while (!trace_file.eof()) {
        ...
        ps->GenerateTraffic2(c.Get(0), ps->packet_size-36, MicroSeconds
                                        ((next_time-first_time).GetMicroSeconds()*0.87), mote1, mote2, mote3);
        //Simulator::Schedule(MicroSeconds(atoi(line.c_str())),
        //                    &ProtocolStack::GeneratePacket, ps, ps->packet_size, curSeqNr++, mote1, mote2, mote3);
  ...
  for (int size = 36; size < 125; size += 7) {
   ...
   // eeh1->Install(ps->deviceFile, c.Get(10));
    ...
    for (int i = 0; i < numberMotes; i++) {
        ...
        auto mote2 = new TelosB(); mote2->Configure(c.Get(i), ps, nullptr);
        auto mote3 = new TelosB(); mote3->Configure(c.Get(i), ps, nullptr);
        ps->GenerateTraffic(c.Get(i), ps->packet_size, mote1, mote2, mote3);
    }
  ...
  //LogComponentEnable("CC2420NetDevice", LOG_LEVEL_INFO);
    ...
    for (uint32_t i = 125; i >= 36; i-=8) {
        ...
        for (int j = 0; j <= 150; j++) {
            ...
            eeh->Install(ps, node_container.Get(0));
            eeh->Install(ps, node_container.Get(1));
            eeh->Install(ps, node_container.Get(2));
