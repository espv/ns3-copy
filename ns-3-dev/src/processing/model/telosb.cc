...
void TelosB::ReceivePacket(Ptr<Packet> packet) {
  ...
  if (receivingPacket) {
    execenv->queues["rxfifo"]->Enqueue(execenv->currentlyExecutingThread->m_executionInfo);
...
void TelosB::receiveDone_task(Ptr<Packet> packet) {
  ...
  if (jitterExperiment) {
     ...
     */
    execenv->queues["ipaq"]->Enqueue(execenv->currentlyExecutingThread->m_executionInfo);
    execenv->queues["ipaq"]->Enqueue(execenv->currentlyExecutingThread->m_executionInfo);
    execenv->queues["rcvd-send"]->Enqueue(execenv->currentlyExecutingThread->m_executionInfo);
    execenv->queues["rcvd-send"]->Enqueue(execenv->currentlyExecutingThread->m_executionInfo);
    execenv->queues["rcvd-send"]->Enqueue(execenv->currentlyExecutingThread->m_executionInfo);
