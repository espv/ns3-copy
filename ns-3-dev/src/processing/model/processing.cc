...
void Processing::HandleQueue2(std::vector<std::string> tokens) {
        ...
        while(i < curPgm->events.size()) {
            ...
            if(curEvt->type == CONDITION) {
                ...
                if(curCnd->condType == LOOPCONDITION &&
                        ((LoopCondition *) curCnd)->emptyQueue2s != NULL) {
                    curPgm = ((LoopCondition *) curCnd)->emptyQueue2s;
                }
                else {
        ...
        if(!tokens[1].compare("QUEUECOND")) {
            ...
            if(!tokens[3].compare("0")){
                if(currentlyHandled->lc == NULL) {
                    ...
                    Ptr<Queue2> firstQueue2 = currentlyHandled->lc->firstQueue2;

                    // First, find the queue name
                    std::map<Ptr<Queue2>, std::string>::iterator name = queueNames.find(firstQueue2);
