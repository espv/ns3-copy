...
namespace ns3
{
    ...
    ResourceManager::Configure(void)
    {
        ...
        StringValue s;
        dcep->GetAttribute("routing_protocol", s);

        if (s.Get() == "olsr") 
        {

            NS_LOG_INFO("Current routing_protocol is olsr");
    ...
    ResourceManager::getRoute(olsr::RoutingTableEntry &rentry) {
        ...
        if (rentry.destAddr == GetObject<Communication>()->GetLocalAddress()) {
            ...
            NS_LOG_INFO(Simulator::Now() << " Destination is local node");
