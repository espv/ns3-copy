...
NS_LOG_COMPONENT_DEFINE ("Dcep");
    ...
    {
        NS_LOG_INFO(Simulator::Now() << " COMPLEX EVENT RECEIVED " << ++number_received << " EVENTS, NOTIFIED HOPSCOUNT " << e->hopsCount << " DELAY " << (Simulator::Now() - e->timestamp).GetMilliSeconds());
    ...
    DataSource::GenerateAtomicCepEvents(std::string eventType){
        ...
        if(m_eventType != " ")
        {
            ...
            NS_LOG_INFO(Simulator::Now() << " CepEvent number  " << e->m_seq);
